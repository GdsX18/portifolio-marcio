...
const OriginButton = React.forwardRef<HTMLButtonElement, OriginButtonProps>(
  (
    ...
    const hasCustomText = className?.includes("text-") || className?.includes("text-[");
    const hasCustomBorder = className?.includes("border-") || className?.includes("border-[");

    const rippleClass = rippleBg || "bg-foreground dark:bg-neutral-50";
    const hoverTextClass = hoverTextColor || "text-background dark:text-neutral-950";
    ...
    const buttonClass = cn(
      ...
      !hasCustomBorder && "border-[0.5px] border-border",
      !hasCustomBg && "bg-card dark:bg-muted",
      !hasCustomText && "text-card-foreground dark:text-foreground",
      "transition-[color] duration-300 ease-[cubic-bezier(0.16,1,0.3,1)]",
      "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background",
      "disabled:pointer-events-none disabled:opacity-50",
      showFill && hoverTextClass,
      className
    );
    ...
    const content = (
      <>
        <motion.span
          ...
          aria-hidden
          className={cn("pointer-events-none absolute -translate-x-1/2 -translate-y-1/2 rounded-full", rippleClass)}
          initial={false}
          style={{
            height: coverSize,
            left: origin.x,
